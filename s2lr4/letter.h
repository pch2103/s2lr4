...
#define BASE_NAME "letterBase1.txt" //Структура файла будет отличаться от базового варианта с одним классом
...
letter* letter::addNewRecord(letter* BS) //выделение памяти под новую структу и заполнение ее данными
{
	...
	else
	{
		...
		{
			T[i] = BS[i];
			BS[i].~letter(); // Удаляем ранние записи из памяти (delete[] LP; - не работает) вызываем деструктор явно для каждой записи
		}
...
{
	showRecordHeader("Все письма в базе:");
...
void letter::showOneRecord(int number = 1) //вспомогательная функция - печать одной записи
{
	...
	std::cout << number;
	std::cout.width(MAX_WIDTH);
	std::cout << std::left << this->getFio().convertFioToString();
	std::cout.setf(std::ios::left);
	std::cout.width(ONE_STRING_WITH);
...
void letter::exitAndSave() // запись базы из памяти в файл при выходе
{
	...
	for (int i = 0; i < amount; ++i) {
		fprintf(FP, "%s\t%s\t%s\t%.2f\n", (this + i)->getFio().convertFioToString(), (this + i)->getInAddress(), (this + i)->getOutAddress() , (this + i)->letterPrice);
	}
